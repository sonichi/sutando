...
- Starting the streaming task watcher via the `Monitor` tool (the step-1.5 command — `bash src/watch-tasks-stream.sh` with `SUTANDO_TASK_EVENT_HANDLER` exported when the worker-pool skill is installed — persistent, description `"Streaming task watcher"`) — **first**, before any cron is registered (2026-08-24: moved ahead of registration so a task arriving during the registration loop isn't queued unprocessed; see `skills/schedule-crons/SKILL.md` step 1.5 for the measured impact)
