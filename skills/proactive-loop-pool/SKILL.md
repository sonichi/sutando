...
Each pool session sets these via its launchd plist (see `scripts/install-worker-pool.sh`):
- `SUTANDO_WORKER_ID` — this session's worker name (`worker-1`, `worker-2`, ...); `src/pool_names.py` owns the spelling.
- `SUTANDO_WORKER_SEAT` — the 1-based seat number behind that name (e.g. `1`, `2`, `3`).
- `SUTANDO_WORKER_POOL_SIZE` — total pool size (informational; not enforced by this skill).
- `SUTANDO_CORE_ID` / `SUTANDO_CORE_POOL_SIZE` — one-release aliases of the seat and size; read only when the `WORKER_*` vars are absent.
- Workspace resolved via `bash scripts/sutando-config.sh workspace` (env override retired post-#1440).

If `SUTANDO_WORKER_ID` (and the `SUTANDO_CORE_ID` alias) is unset, abort with a clear error: "proactive-loop-pool requires SUTANDO_WORKER_ID — are you sure you meant to invoke this instead of /proactive-loop?"

## Activation (persistent follower form)

Followers are long-lived interactive sessions inside tmux (`tmux attach -t worker-N` to observe), started by the launchd wrapper. On activation:
...
2. Start the streaming task watcher via the `Monitor` tool — `command: 'bash src/watch-tasks-stream.sh'`, `persistent: true`. React ONLY to events for your own assignments (`task-*.assigned-$SUTANDO_WORKER_ID.txt`) plus, in leaderless fallback, unassigned `task-*.txt` (acquire_work decides — see below). Ignore other instances' assigned/claimed events.
3. Liveness is the wrapper's job (`pool-follower-beat.sh` writes `state/cores/worker-N.alive` pid-bound to this session) — do not start `core_heartbeat.py` in-session.
...
2. Run: `python3 src/pool_follower (acquire_work).py <id> $SUTANDO_WORKER_ID`
...
2. Peek at the task body (without claiming yet) to read the `channel_id:` line. (Use `head` / `grep` — don't run a full Read tool yet since other workers may grab the task first.)
3. Run: `python3 src/claim_task.py <id> $SUTANDO_WORKER_ID <channel_id>`
4. Outcomes:
   - **Exit 0** → claim won. Script prints the renamed path (`tasks/task-<id>.claimed-worker-<n>.txt`). Read THIS path. Your worker is now the channel's handler for the next 30 min (default `SUTANDO_CORE_IDLE_THRESHOLD_SEC`).
   - **Exit 1** → respect-handler OR lost-race. Either another worker is the channel's active handler, or another worker won the race-claim. Skip this task entirely.
   - **Exit 2** → validation error. Log and skip.

The affinity machinery is **inside** `claim_task.py` — your only responsibility is to pass `channel_id` when present. The script reads `state/cores/channel-<id>.handler` and `state/cores/worker-<n>.alive` to decide whether you're allowed to claim.

Use the renamed `task-<id>.claimed-worker-<n>.txt` path for all subsequent reads + result writes.

**Core attribution (required, owner request 2026-08-23):** end every user-facing result body with a final line naming your worker, em-dash form: `— worker-<n>` (formerly core-<n>; owner renamed 2026-08-31 — one core, N workers). Plain text only — never a bracketed form (`[worker-N]` would trip ag2space's `team_result_guard`, which withholds bodies carrying bracketed control markers). Skip the signature only on `[deduped:]`/`[no-send]` bodies, which no user reads.
...
Without it, three follow-up Discord messages on the same topic would scatter across the 3 workers. Each worker would run a partial proactive-loop pass and write a partial result; the latest task's result would carry the consolidated reply via `[deduped:]` marker. That's wasteful — 3× quota burn for 1× user-visible reply.

With affinity, all 3 tasks land on the same worker; that worker has the conversational context in its session memory and produces one coherent reply. Quota burn matches what a single-core setup would have done.
...
- `reclaim_dead()` / `reclaim_stuck_assignments()` — an assignment a dead or
  wedged worker never claimed goes back to the pool.
- `reclaim_claimed()` — a claim held by a worker that died is restored to its
...
*alone* means the worker died between flagging and writing the result — nothing
...
1. Remove `SUTANDO_WORKER_POOL_SIZE` from `.env` (or set to 1).
2. Run `bash scripts/uninstall-worker-pool.sh` to remove the launchd plists.
...
got = acquire_work(WORKSPACE/"tasks", WORKSPACE/"state",
                   os.environ["SUTANDO_WORKER_ID"], "pool-lead")
```
