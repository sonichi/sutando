...
#     rsynced while writers are live; voice-metrics.jsonl + call-metrics.jsonl
#     are frozen archives with no new writes since #603; latency.json /
#     scanned-calls.json are per-node metrics that don't need consolidation.
#     Nothing remaining in data/ benefits from cross-node sync.)
...
# (data/ is intentionally NOT defined here — see header comment for why
# the whole data/ dir is per-node, not synced.)
...
RSYNC_FLAGS=(-az --update
    ...
    --exclude 'asset-library/')
# asset-library/ is bulk video-production cruft (mp4/zip/pdf render artifacts,
# 19GB+) that lives under notes/ but has nothing to do with cross-node text
# learning. Syncing it made every pass push ~19GB over the link ("never
# completes"). Excluded per Susan 2026-06-01 — notes sync is for text knowledge,
# not media assets.
...
# sync"). conversation.sqlite cannot be safely rsynced while writers are
# live (`integrity_check` damage signature: "2nd reference to page" /
# "rowid out of order"); the rest of data/ is frozen archives or per-node
# metrics. If cross-node consolidation of conversation.sqlite becomes
# necessary, the right shape is `.backup`-snapshot-based sync, not rsync.
