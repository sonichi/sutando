...
## Troubleshooting — silent no-op (root-caused 2026-06-10)

If the script prints "Sync complete" (exit 0) but nothing arrives on the peer,
check these three in order — they stacked and hid each other for ~3 weeks:

1. **Run the REPO copy, never the `~/.claude/skills/` copy.** `REPO_ROOT` is
   derived from the *script's own location* (three dirs up). Invoked from
   `~/.claude/skills/...`, REPO_ROOT becomes `~/.claude` and every local path
   (MEM_LOCAL slug, notes, assets, the auto-loaded `.env`) is garbage.
2. **Errors are swallowed by design.** `run() { "$@" || true; }` keeps one
   failed pass from killing the rest — but it also means "Sync complete" is
   NOT proof of transfer. After any run that matters, verify on the peer:
   `ssh $SUTANDO_SYNC_PEER ls <peer-mem-dir>/<some-fresh-file>.md`.
3. **Set `SUTANDO_WORKSPACE` in `.env`** (even to its default value) or the
   notes pass silently sources the legacy repo `notes/` instead of
   `<workspace>/notes/` — new workspace notes never leave the machine.

Peer layouts differ (e.g. Studio `/Users/xueqingliu/...` vs MBP
`/Users/xliu/Documents/xqq/...`): `SUTANDO_PEER_MEM_DIR` / `SUTANDO_PEER_NOTES_DIR`
in `.env` are REQUIRED in that case — the same-path defaults silently fail
(see item 2 for why you won't notice).
