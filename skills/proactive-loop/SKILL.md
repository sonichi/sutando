...
1. **Catchup first** (fresh-session only). Resolve `WORKSPACE="$(bash scripts/sutando-config.sh workspace)"` (M0 helper, PR #1395), then PID-check `"$WORKSPACE/state/proactive-loop-started.sentinel"`: read its content as a PID and verify it's alive (`kill -0 "$pid" 2>/dev/null`). If sentinel is absent OR its PID is dead (stale from a prior session that crashed before SessionEnd cleared it) → run `/catchup-after-startup` BEFORE anything else so the conversation buffer has cross-restart context, then `mkdir -p "$WORKSPACE/state"` and `echo "$PPID" > "$WORKSPACE/state/proactive-loop-started.sentinel"` (PPID = the claude CLI process; stable across Bash tool calls within a session). If sentinel exists AND its PID is alive → this is a cron-driven pass within the running session; skip catchup and proceed. The sentinel is also cleared by the SessionEnd hook (`src/session-handoff.sh`, which now resolves via the same helper) on clean exit; the PID-alive check is the safety net for unclean exits (crash, OOM kill, force-quit) where the hook never fires. Explicit reset still works: `rm "$WORKSPACE/state/proactive-loop-started.sentinel"`. **Important:** the prior `${SUTANDO_WORKSPACE:-$HOME/.sutando/workspace}` form is no longer correct post-M0 (would write/check the legacy default while the handoff clears the M0 default → catchup silently skipped); always use the helper here.
