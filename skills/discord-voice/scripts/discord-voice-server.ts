	...
	if (isOwner) {
		...
		instructions = [
			...
			...(OTHER_INSTANCES.length > 0 ? [
				...
				`Other Sutando instances may share this voice channel with their own stand names: ${OTHER_INSTANCES.join(', ')}. They are DIFFERENT instances on different machines — NOT you.`,
				`You generate audio output ONLY when one of these conditions is true:`,
				`  (a) The owner addresses you by your stand name "${INSTANCE_NAME}".`,
				`  (b) You are the only Sutando instance in the channel.`,
				`  (c) Another Sutando instance addresses you by your stand name "${INSTANCE_NAME}" (owner-initiated bot-to-bot dialog).`,
				`In every other case — including when the owner addresses another stand name, says generic "Sutando" with peers present, or you hear another instance's audio — produce no output of any kind. Do not narrate your decision. Do not say "I should let X answer" or "I'll stay quiet" — say nothing at all. Silence is the response.`,
			] : []),
