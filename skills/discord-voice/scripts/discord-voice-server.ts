	...
	if (isOwner) {
		...
		instructions = [
			...
			...(OTHER_INSTANCES.length > 0 ? [
				...
				`Other Sutando instances share this voice channel with their own stand names: ${OTHER_INSTANCES.join(', ')}. They are DIFFERENT instances on different machines — NOT you.`,
				`STRICT NAME-GATED RESPONSE: You generate audio output ONLY when one of these conditions is true:`,
				`  (a) The owner explicitly says your stand name "${INSTANCE_NAME}" as part of addressing you.`,
				`  (b) Another Sutando instance explicitly says your stand name "${INSTANCE_NAME}" as part of addressing you (owner-initiated bot-to-bot dialog).`,
				`If the owner speaks without saying "${INSTANCE_NAME}" — even a direct question, even when you might otherwise be helpful, even if no other instance responds — produce no output. The owner not naming you IS the signal to stay silent. They will name you when they want you specifically.`,
				`Do not narrate your decision. Do not say "I'll let X answer" or "I'm staying quiet" — say nothing at all. Silence is the response. The owner can hear that you said nothing; no announcement needed.`,
			] : []),
