...
(`<workspace>` resolved via `bash scripts/sutando-config.sh workspace` — defaults to `<repo>/workspace/`, honors `$SUTANDO_WORKSPACE` as legacy escape hatch).
...
Operational/diagnostic output (the `[Setup]`/`[Voice]`/`[Tool]`/`[VoiceSession]`/`[Dismiss]` lines) is tee'd to `<workspace>/logs/discord-voice.log` — the discord-voice counterpart to `<workspace>/logs/discord-bridge.log` and `<workspace>/logs/voice-agent.log` — so the operational history survives a process exit.
