...
5.6. **Auto session-recap on boot (owner directive 2026-07-13).** If `recap_room` is set in this host's `recap.json` (sibling file — `crons.json` itself stays a bare job list) AND more than one session transcript exists, run the `session-recap` skill in brief mode over the previous session and post to `recap_room` (gateway op:message). Idempotence lives in the recap skill's `state/last-recap-session.txt` stamp — a mid-session `/schedule-crons` re-run finds the previous session already stamped and skips, so this never double-posts (same guard philosophy as the dynamic-loop freshness sentinel in step 3).
