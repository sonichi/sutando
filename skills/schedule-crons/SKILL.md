...
5.5. **Ensure the core heartbeat is running (sonichi/sutando#2198 prerequisite).** `src/core_heartbeat.py` (the writer of `state/cores/<hostname>.alive`) is started by `src/startup.sh`, but the CLI boot path and `start-cli.sh --restart` land here without it. Freshness of the file is NOT the running-check: a writer orphaned by a previous session keeps it fresh and records the new pane's pid into it. Run the gate, which reads the record's content the way the writer wrote it:
   ```bash
   python3 skills/schedule-crons/scripts/heartbeat-gate.py
   ```
   It prints one word. `start` (no file, stale, `pid` is not this core's pane, or `heartbeat_pid` is not a live writer — a live one the record named has already been stopped) → `nohup python3 src/core_heartbeat.py > /tmp/core-heartbeat.log 2>&1 &`. `skip` (fresh, this pane, writer alive) → nothing. `unknown` (exit 2: tmux or ps did not answer) → report it in step 6 and do not start blind. Exit 3 → the named writer would not stop; run the command it prints, then re-run the gate. Do NOT use `pgrep -f core_heartbeat` (the wrapper-argv self-match of step 1.5's watcher note) and do NOT fall back to the mtime rule.
