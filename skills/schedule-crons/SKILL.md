...
0. **Catchup first (fresh-session only).** Resolve `WORKSPACE="$(bash scripts/sutando-config.sh workspace)"` (M0 helper) and PID-check `"$WORKSPACE/state/proactive-loop-started.sentinel"`: read its content as a PID and verify the process is alive (`kill -0 "$pid" 2>/dev/null`). If the sentinel is absent OR its PID is dead (stale from a prior session that crashed before SessionEnd cleared it) AND the `catchup-after-startup` skill is installed (i.e. `$CLAUDE_CONFIG_DIR/skills/catchup-after-startup/` exists) → invoke `/catchup-after-startup` BEFORE the cron-scheduling work below, so the conversation buffer carries cross-restart context (last session's PRs, in-flight tasks, recent voice/discord activity, build-log tail) before any cron fires. After invocation, `mkdir -p "$WORKSPACE/state"` and `echo "$PPID" > "$WORKSPACE/state/proactive-loop-started.sentinel"` (PPID = the claude CLI process; stable across Bash tool calls within a session). If the sentinel exists AND its PID is alive (cron-driven re-invocation within the running session) OR catchup skill not installed → skip step 0 silently and proceed. The sentinel is also cleared by the SessionEnd hook in `src/session-handoff.sh` (per `6e5c58d` in #1056) on clean exit; the PID-alive check is the safety net for unclean exits (crash, OOM kill, force-quit) where the hook never fires.
