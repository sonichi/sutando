...
4. **Fallback — ensure `/proactive-loop` is scheduled.** After step 3, check whether any job in `crons.json` **that step 3 could actually register** references `/proactive-loop` (either `"prompt_skill": "proactive-loop"` or a `"prompt"` whose body invokes the loop). **A job step 3 skipped does not count** — a `monitor` entry, one owned by launchd or the Codex runner, or one carrying `shell_command` (which the session cannot run at all). Otherwise the reference alone suppresses the fallback while nothing registers the driver: `{"name":"main-loop","cron":"*/5 * * * *","shell_command":"echo hi","prompt_skill":"proactive-loop"}` is skipped by step 3 for its shell key, skipped by cron-runner for having no `launchd` marker, and would suppress the fallback on its `prompt_skill` — leaving the session with NO recurring driver at all. Read runnability from `select_for_executor(entry, EXECUTOR_FORMS["session"])`, the same policy step 3 and `health-check`'s `session-crons` probe use, not a second reading of the keys. If none does, call `CronCreate` directly with `cron: "*/10 * * * *"` and `prompt: "/proactive-loop"` as a bootstrap-safety net. Rationale: post-#954 the CLI boots with `-- "/schedule-crons"` and exits after step 5, so if `crons.json` is missing/empty/forgot-to-include-the-loop-entry the session goes idle with no recurring work driver. The fallback guarantees the loop runs at least every 10 min regardless of config state. Idempotent: if the user has a custom `*/5 * * * *` or `*/15 * * * *` entry, that satisfies the check and the fallback is skipped (no duplicate cron).
