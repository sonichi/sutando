...
5.6. **Auto session-recap on boot (owner directive 2026-07-13; taken OFF the boot critical path 2026-07-31).** When more than one session transcript exists (i.e. there is a previous session to recap), launch the `session-recap` skill's boot recap over the previous session **as a BACKGROUND subagent — never inline in the boot turn**. Spawn it with the Agent tool (`model: haiku`, `run_in_background: true`) AFTER the watcher (step 5) is armed; the boot turn then ends and the session is immediately responsive to owner tasks. Rationale + measurements: previous transcripts run 25–58 MB, and generating the recap inline dominated boot (the owner's reported ~4-minute cold start — see PR for live timings); with the recap in the background, boot-to-responsive is bounded by service checks + cron registration only. Until the deep recap lands, catch up from `state/current-track.md` + the latest `relay/relay-*.md` (both cheap, both maintained every substantive pass). The background subagent runs the recap contract exactly as before — bound its dump with `--tail-bytes 8388608` (8 MiB, the recap contract's boot default) so the read cost stops scaling with session length. Per the recap contract (`skills/session-recap/SKILL.md` "Automatic recap on restart"), this is **two behaviors with different gates** — do NOT gate the whole step on `recap_room`:
   ...
   **Single-flight gate (required BEFORE spawning).** The completion stamp (`state/last-recap-session.txt`) is written by the worker only at the END of its ~1–2 min background window, so the stamp alone cannot protect a mid-session `/schedule-crons` re-run inside that window from launching a second worker (two summarizers racing `last-session-recap.md` + a duplicate room post). Reserve the session atomically first: run `python3 skills/session-recap/scripts/recap_claim.py claim <previous-session-uuid>` and spawn the background subagent ONLY on exit 0. The winning claim prints `claimed token=<nonce>` — capture that token and pass it into the worker's prompt. Exit 1 = already recapped or a worker is in flight — skip the spawn entirely. Include in the worker's prompt that it must refresh its lease with `recap_claim.py renew <uuid> --token <nonce>` after extraction, after summarization, **immediately before writing `last-session-recap.md`, and again immediately before the optional private-room post**. Every renew must exit 0; on any nonzero exit the worker lost ownership and MUST stop without writing/posting. It must finish with `recap_claim.py release <uuid> --token <nonce> --stamp` on success, or `release <uuid> --token <nonce>` (no `--stamp`) on failure so a later boot retries. Renew and release are ownership-checked against the on-disk token: a worker that stalls past the 15-minute lease and is reclaimed cannot resume into either output side effect or stamp/release the successor's run. The claim is O_EXCL-atomic (exactly one winner among overlapping invocations); crashed workers become reclaimable, while live workers keep their lease fresh at bounded milestones.
