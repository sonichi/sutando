...
The skill both **uses** and **maintains** `<workspace>/hosts/<hostname>/current-track.md` (NOT the legacy flat `state/current-track.md` — writing there again re-creates the cross-host clobber) — the owner doesn't dictate its content and the agent doesn't invent it from memory; it's **derived from the reconstruction**:
...
- 2026-08-03 PATH MOVED, after a live data loss: the anchor now lives at `<workspace>/hosts/<hostname>/current-track.md`, not the flat `state/current-track.md`. The flat path was added to the shipped carrier set by #2534 and is **shared across hosts**, so two cores wrote the same vault path: a peer enabling the carrier overwrote this host's 1056-line anchor, and the peer's own correct remediation (`git rm --cached`) then deleted the restored copy. Three writes, each individually correct, all destructive. `hosts/<label>/` is already carried by `hosts/*/`, so the per-host path needs no carrier entry and cannot collide — structurally impossible rather than correctly configured (#2567; refinement credit: Sutando-Mini). Note the entry above is left as written — it was true on 2026-06-25.
