...
# The service's own grammar. Validated here too, so an invalid kind fails
# before it can be dropped from the URL and silently mean "the markdown one".
...
def doc_socket_url(api_root: str, room_id: str, kind: str = DEFAULT_KIND) -> str:
    ...
    if not KIND_RE.fullmatch(kind or ""):
        raise RoomDocError(
            f"document kind must match {KIND_RE.pattern} — got {kind!r}. "
            "An empty or malformed kind would silently open the markdown document.")
