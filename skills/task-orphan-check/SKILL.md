...
It prints one JSON object — `tasks[]` with `id`, `source`, `access_tier`, `channel_id`, `label` (the readable channel label step 3b uses), `age_s`, `import` (with `import_intent`; `import_task_id` — the `task_id` the current `status.json` carries, `null` when it has none; and for a bound or unbound run `import_phase` / `import_idle_s`), `verdict` (`done` / `worker-held` / `unknown` / `fresh` / `orphan` / `import-resume` / `import-stalled` / `import-unbound`; a `worker-held` row also carries `holder`, the recipient id) and a `reason` naming the marker, sentinel or age line it matched — plus `counts`. Read-only: it moves and writes nothing; step 3 acts on its verdicts. The rules it encodes are the ones below (pinned by `tests/task-orphan-check-classify.test.py`); if you change a rule, change the script and its test, then this prose.
...
2. **Cross-reference completion markers** (any single match = task already completed):
   ...
   **Step 2c — a task a worker holds is not the core's orphan.** The pool router delegates a task by writing a sentinel `deliveries/<recipient>/<id>{.txt,.accepted,.claimed}`; the payload stays in `tasks/`, where the recipient reads it. Once ANY worker holds a sentinel — even an unaccepted one — the task is that worker's to answer, and the core's age line says nothing about it: on 2026-09-18 the boot pass archived four owner DMs held by two worker seats (6–26 min old, no result yet because the workers were mid-answer) and reported them to the owner as unanswered. The predicate is `skills/worker-pool/scripts/worker_delivery.py:holder_of` (the worker-pool skill is optional: absent skill ⇒ no router ⇒ nobody holds anything) — the same one the Stop hook (`src/check-pending-tasks.sh`) and `scripts/unanswered-tasks.py` apply — never a local re-scan of `deliveries/`. It answers `None` only when the directory or sentinel is genuinely absent (ENOENT); any other stat failure propagates, and the classifier turns that into **UNKNOWN**, because reading an unreadable `deliveries/` as "nobody holds it" is exactly what hands a worker's task to the core. A completion marker (step 2) still wins: a held task with a result is done.
...
orphan-check complete:
  ...
  left for its worker (worker-held): W
  left in place, deliveries/ unreadable (unknown): X
...
The summary lands in the conversation buffer so the agent's first turn (and operator) sees what happened. If `M+K+W+X+I+S+U+J ≠ N`, the script bailed mid-pass — log a warning and let the operator investigate.
...
- v0.1.10 — 2026-09-18 — #4399 review (keweichen, qingyun-wu; qingyun-wu reversed an approval to file it). v0.1.9's `preview` copied untrusted task text into the trusted `proactive-orphan-recovery-*` result, and `src/result_markers.py` reads `[file:]`/`[send:]`/`[attach:]` anywhere outside code as an attachment action that the proactive Discord path executes — so a non-owner could seed an orphan task naming an allowlisted local file and have startup recovery upload it to an external chat service. keweichen demonstrated it through the production parser. `classify.py:neutralize()` now turns `[`/`]` into `(`/`)` in `preview` and `label` before either leaves the row, which removes the structural precondition of every marker without duplicating the grammar the repo keeps in one place. Pinned by `TestPreviewMarkerNeutralization`: all three aliases through the real step-3b body shape assert no `attach` action, with a positive control proving the parser DOES fire on the un-neutralized text (so the zero is a finding, not a vacuum). Reader-visible consequence: a bracketed prefix now shows as `(Discord @name)`.
- v0.1.9 — 2026-09-18 — step 3b's preview moved into `classify.py` (`preview` per row; `PREVIEW_CHARS = 100`). The prose snippet stripped a system-instructions block it assumed sat at the FRONT of the file and then sliced the body; the bridges put `task:` last and append the block after it, so the 2026-09-18 boot's 28-orphan recovery DM previewed every task as `id: task-… envelope_hmac: v1:…`. The classifier now emits the `task:` value cut before the marker, whitespace-collapsed, and the prose reads that field instead of re-deriving it; pinned by `TestPreview` against the real task-last shape, the task-mid shape, and a file with no block.
- v0.1.8 — 2026-09-18 — worker-held tasks. The classifier consulted results/ and age only, so on the first boot after a core restart four owner DMs (6–26 min old) that the pool router had already handed to two worker seats — `deliveries/<recipient>/<id>{.txt,.accepted,.claimed}` present, no result yet because the workers were mid-answer — classified `orphan`, were archived out from under their workers and reported to the owner as unanswered. New step 2c: after the completion-marker check, `skills/worker-pool/scripts/worker_delivery.py:holder_of` (the Stop hook's and `unanswered-tasks.py`'s predicate) → **WORKER-HELD** (left in tasks/, never archived, never DM'd; row carries `holder`); a non-ENOENT failure reading deliveries/ → **UNKNOWN** (left in place, listed in the DM), since an unreadable deliveries/ read as empty is what hands a worker's task to the core. Pinned: every router suffix, any age (also under the 300 s line), result beats sentinel, sentinel beats the import branch, the unreadable case, a control with the recipient dir but no sentinel, and a control with no deliveries/ at all.
