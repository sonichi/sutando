...
- **Scope**: `hooks/`, `packages/ag2-sparrow/`, `src/`, `scripts/`, and
  `skills/` Python (see `.coveragerc`). Package files generated byte-for-byte
  from canonical `src/` modules are omitted; the canonical file is measured
  and `tests/ag2-sparrow-drift.test.py` proves the package copy is identical.
