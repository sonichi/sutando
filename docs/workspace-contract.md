...
- **Team behavioral guardrail + outbound scan** — Team is a trusted-collaborator tier, not an adversarial isolation boundary. Its prompt requires scoped, cautious work and forbids disclosing credentials or unrelated owner context. Before a Team result is published, the handler rejects bridge delivery-control markers, runs the maintained chat-secret filter, and withholds results containing likely credentials. This reduces accidental response leakage but does not prevent side effects, make private workspace content unreadable, or guarantee detection of every sensitive value. Operators should grant Team only to collaborators they trust with the configured workspace and integrations.
