...
- **Per-channel access tiers** — every bridge tags incoming tasks with `access_tier: owner | team | other`. AG2 Space rooms whose broker-attested setting is explicitly `team` also receive a trusted pre-body execution stamp and run in a fresh instance of the owner's selected runtime with normal configured capabilities. Missing/invalid stamps, local owner-to-Team caps, Discord/Slack/Telegram Team, and `other`/Guest retain the read-only delegated path. See CLAUDE.md access-control sections.
