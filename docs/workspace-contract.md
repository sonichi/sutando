...
- **Per-channel access tiers** — every bridge tags incoming tasks with `access_tier: owner | collaborator | team | guest` (`other` is the legacy spelling of `guest`, `ambient` of `guest` + `origin: promoted`; readers still accept both). AG2 Space Team plus an explicit per-agent Agent Native `collaborator: true` control receives a trusted pre-body execution stamp and runs in a fresh instance of the owner's selected runtime with normal configured capabilities. Team without that control, missing/invalid stamps, local owner-to-Team caps, Discord/Slack/Telegram Team, and Guest (legacy `other`) retain the restricted path. See CLAUDE.md access-control sections.
...
- **Team behavioral guardrail + outbound scan** — opted-in AG2 Space Team is a trusted-collaborator tier, not an adversarial isolation boundary. Its prompt requires scoped, cautious work and forbids disclosing credentials or unrelated owner context. Before a Team result is published, the handler rejects bridge delivery-control markers, runs the maintained chat-secret filter, and withholds results containing likely credentials. This is an accidental final-response leakage check only: it cannot observe or prevent a malicious or successfully prompt-injected task from reading credentials, sending them over the network, changing files or git state, invoking a webhook, or causing any other side effect before returning a scanner-clean response. Owners should grant Team only in rooms whose Team members they trust with everything available to the configured workspace, runtime, and integrations.
- **Future AG2 Space monitoring** — a later defense-in-depth layer can aggregate security telemetry across agents and surfaces, detect prompt-injection/anomalous-tool patterns, correlate incidents, alert owners, and revoke credentials or agent sessions quickly. This is a roadmap direction, not a capability or security guarantee of the current Team implementation.
