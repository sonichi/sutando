...
- **Per-channel access tiers** — every bridge tags incoming tasks with `access_tier: owner | team | other`. AG2 Space Team plus an explicit per-agent Agent Native `collaborator: true` control receives a trusted pre-body execution stamp and runs in a fresh instance of the owner's selected runtime with normal configured capabilities. Team without that control, missing/invalid stamps, local owner-to-Team caps, Discord/Slack/Telegram Team, and `other`/Guest retain the restricted path. See CLAUDE.md access-control sections.
