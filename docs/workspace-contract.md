...
- **Per-channel access tiers** — every bridge (Discord, Slack, Telegram) tags incoming tasks with `access_tier: owner | team | other`. Explicit `team` tasks run in a fresh instance of the owner's selected runtime with the normal configured workspace and capabilities, preceded by a trusted Team guardrail that identifies the requester as a collaborator rather than the owner. `other`/Guest retains the read-only delegated path. See CLAUDE.md "Discord/Slack/Telegram access control."
...
- **Team behavioral guardrail + outbound scan** — Team is a trusted-collaborator tier, not an adversarial isolation boundary. Its prompt requires scoped, cautious work and forbids disclosing credentials or unrelated owner context. Before a Team result is published, the handler rejects bridge delivery-control markers, runs the maintained chat-secret filter, and withholds results containing likely credentials. This is an accidental final-response leakage check only: it cannot observe or prevent a malicious or successfully prompt-injected task from reading credentials, sending them over the network, changing files or git state, invoking a webhook, or causing any other side effect before returning a scanner-clean response. Operators should grant Team only to collaborators they trust with everything available to the configured workspace, runtime, and integrations.
