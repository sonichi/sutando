...
The sandboxed agent is Codex by default. On an install without Codex CLI, set `sandbox.runtime` to `gemini` in `sutando.config.json` (or `SUTANDO_SANDBOX_RUNTIME=gemini`) and the same two-stage rulebook delegates to the Gemini CLI instead, read-only and sandboxed, through `skills/claude-gemini/scripts/gemini-sandbox.sh`. Without either CLI, every non-owner task ends in the Stage-2 fallback sentinel. See [docs/gemini-sandbox.md](gemini-sandbox.md).
