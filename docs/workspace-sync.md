...
**Customize per workspace** — `sync-workspace.sh` reads `vault.sync.include` and `vault.sync.exclude` from `sutando.config.local.json`. ⚠ **`include` REPLACES the default carrier set wholesale — it does not add to it.** Config arrays are replaced, not merged (`_deep_merge` in `src/sutando_config.py`; pinned by `tests/sutando-config.test.py::test_local_replaces_arrays_wholesale`). The carrier is a **whitelist** — `*` ignores everything, then the include list is un-ignored — so an override that lists only your extra paths silently drops `notes/`, `hosts/*/` and the whole memory corpus out of the backup, **while sync keeps reporting success**. To add a path you must restate the full default set alongside it. `exclude` does subtract, carving subpaths out of an included parent. The carrier writes these patterns into `<workspace>/.git/info/exclude` on each sync tick (mechanism: [#1447](https://github.com/sonichi/sutando/pull/1447) + [#1460](https://github.com/sonichi/sutando/pull/1460) — not the workspace's `.gitignore`, to avoid leaking the carrier-set rules into operator-tracked state).
