...
| `REMOTE_PROACTIVE_ROOM` *(.env too)* | no | — | Bootstrap room for `results/proactive-*.txt` nudges (`POST /v1/room` op:message, claim-by-rename, archive on success): used only until the gateway has answered `GET /v1/agents` with this agent's `owner_dm_room`; that reading is then the owner-private destination, kept on disk (`state/owner-routing.json`) across restarts and outages, and never replaced by an answer without one. Unset → read from this instance's `channels/<dir>/.env`. A file naming its own room (`[channel: !room]`) never needs it; the drain runs whether or not it is set. |
