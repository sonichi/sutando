...
- **Push fails over SSH / headless session (macOS Keychain error)** — If the memory repo uses an `https://` URL and your git credential helper is `osxkeychain` (the macOS default), `git push` requires an unlocked GUI session. Over SSH terminals, tmux-over-SSH, or cron jobs outside the GUI session the Keychain is unavailable and the push silently fails or hangs. Two fixes:
  1. **SSH remote URL (recommended for headless/multi-machine):** change `SUTANDO_MEMORY_REPO` in `.env` to the SSH form: `git@github.com:your-org/your-memory.git`. SSH uses key-based auth — no Keychain involved.
  2. **GH_TOKEN env var:** add `GH_TOKEN=<your-pat>` to `.env`. `git` and `gh` both honour this variable and bypass the Keychain when it is set.
