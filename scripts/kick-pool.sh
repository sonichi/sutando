...
#
# Walks the pool's tmux sessions. Per session: resolve that worker's runtime from
...
# REPL, dismiss a menu, never overwrite other staged input, skip a busy worker).
...
#
# Session naming and socket must match scripts/pool-worker-wrapper.sh — the
# wrapper creates `worker-N` on tmux's DEFAULT socket. An override is accepted
...
if ! [ -r "$DRIVE_LIB" ]; then
  echo "kick-pool: missing $DRIVE_LIB — re-run scripts/install-worker-pool.sh" >&2
  exit 2
fi
...
SESSION_PREFIX="${SUTANDO_POOL_SESSION_PREFIX:-worker-}"
...
# A DEAD worker has no session, so the pane walk below cannot see it. launchd
...
for sess in ${sessions:-}; do
  # The worker's own plist is the only authority on which runtime it runs. An
  ...
  if pool_drive_kick "$runtime" "$sess" tmux_cmd "$sess"; then
    kicked=$((kicked+1))
  fi
done

echo "kick-pool: kicked $kicked worker(s)"
