...
#   1. No .git -> not a checkout, so every field below would be None anyway. The
#      packaged app ships the engine as an rsync copy WITHOUT .git, so on that
#      install these five calls did no useful work at all.
