...
echo "--- Test 4a: Source code bug detection (old one-liner present?) ---"
...
if [ "$CURRENT_EXISTSSYNC" -ge 1 ]; then
    pass "FIX APPLIED: new logic with existsSync(SRT) guard is live"
else
    fail "BUG PRESENT: subtitled_pending has no SRT/existsSync guard — PR #355 not merged"
fi
echo ""
