...
core). Exit 0 only if start, restart, and BOTH probes succeeded.
EOF
}

LABEL="" START="" RESTART="" PROBE="" READY="" TARGET="local" KNOWN_HOSTS=""
...
if [ "$ARC" -eq 0 ]; then
  echo "PASS ✅ — start, restart, and both probes succeeded; behavior round trip verified"
  exit 0
else
