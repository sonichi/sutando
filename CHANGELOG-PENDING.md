...
<!-- feat() PRs go here -->
- `sandbox.runtime` config option (`codex` default, `gemini`): on installs without Codex CLI, non-owner (team/guest) Discord tasks delegate to the Gemini CLI, read-only and sandboxed, through the same two-stage rulebook instead of ending in the Stage-2 fallback sentinel. `SUTANDO_SANDBOX_RUNTIME` overrides per-invocation. ([#3760])
