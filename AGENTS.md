...
Beyond an explicit "at background", spawn a subagent when (Chi 2026-08-21, ag2space):
...
**Specify the model on every spawn — never leave it implicit.** Cheap tier (e.g. `model: haiku`)
for mechanical extraction, searching, formatting, monitoring, and straightforward test runs; inherit
the session model for judgment-heavy work (architecture, ambiguous debugging, security/privacy
decisions, code review, synthesis). Escalate a cheap-model result to the session model when
confidence is low or consequences are material; never silently choose a more expensive model.
