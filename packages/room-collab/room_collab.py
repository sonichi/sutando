...
# Set by the host that installs this package (the Sutando skill sets both);
# the package itself names no host layout.
WORKSPACE_RESOLVER: Callable[[], str | Path] | None = None
ROOM_OPS_SCRIPT: Path | None = None
...
def _workspace(explicit: str | None) -> Path:
    ...
    if WORKSPACE_RESOLVER is None:
        raise RoomDocError("no workspace: pass --workspace (no host resolver is installed)")
    return Path(WORKSPACE_RESOLVER())
