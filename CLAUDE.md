...
**Before coordinating work with a person or agent, invoke [`skills/collaboration-intelligence/`](skills/collaboration-intelligence/SKILL.md).** It owns the cross-channel map of rooms, identities, agent→owner links, and who actually owns or reviews a component — so *whom to ask* is derived from evidence rather than guessed. Invoke it whenever you must recruit a reviewer, delegate, hand off, escalate, resolve an identity across bridges, or decide where to ask. Two failure modes it exists to prevent:

- **Recall is not the method.** A memory that answers "who do I ask?" is one past situation's cached answer; it does not know whether it covers the question in front of you. When one fires, treat it as a candidate and invoke the skill anyway — a reviewer set copied from another PR looks like continuity and is a guess.
- **A bare room post is not an ask.** Addressing (reply-to or @-mention) is what hands the recipient's agent a task; an unaddressed message is context only and recruits no one. Conversely, when you are *publishing* context rather than asking, a bare post is correct — @-mentioning to be polite manufactures an obligation nobody owes.
