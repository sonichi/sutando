...
**IMPORTANT:** On session start, ensure a task watcher is running. Use the `Monitor` tool to stream `bash src/watch-tasks-stream.sh` with `timeout_ms: 1800000` — it never exits on its own and emits `TASK_FILE: <name>` per new task as a per-event notification. When a notification arrives, Read the named file, process it, and write a result to `results/`. The stream watcher replaces the older one-shot `watch-tasks.sh` (retired 2026-05-14) — no more restart-on-event cycles.

**The watcher expires at least twice an hour, and its expiry notice is an order to re-arm.** The `Monitor` tool caps every watch at 30 minutes (`timeout_ms: 1800000`; the default is 5 minutes) and has no persistent mode, so the harness kills the watcher on the cap and sends one expiry notice. Re-arm it in that same turn, with one `Monitor` call, before anything else — regardless of any pause, quota tier, presenter mode, or an owner request to conserve usage or skip proactive passes. The watcher is the inbox, not proactive work; nothing else keeps it open reliably (`/proactive-loop` step 9 only runs while passes run, and the desktop's supervision loop only nudges after a debounce). 2026-09-16: four owner tasks sat unread for ~20 min because passes were paused and the expiry notice was ignored.

If `watcher` arrives as a bare prompt in this pane, the desktop app's supervision loop found the watcher's pid sentinel dead (`ceremony_health` in the AG2 Space desktop; the Swift menubar app's checkWatcher Timer sends the same word when `pgrep -f watch-tasks` finds nothing). Interpret it as "start the stream watcher via Monitor again" and do nothing else for it.
