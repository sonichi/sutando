...
If you notice the stream watcher has stopped, re-arm it yourself via the `Monitor` tool as described above. The two hosting modes (your session watcher, or the supervisor's external standby), the readiness proof and the handoff rules: [`docs/task-watcher-hosting-modes.md`](docs/task-watcher-hosting-modes.md).
