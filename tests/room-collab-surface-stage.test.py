...
async def test_the_relay_switches_surface_and_acts_on_the_one_it_holds():
    ...
    port = 47823
    task = asyncio.create_task(serve(open_doc, port, log=lambda *_: None, open_kind=open_kind))
    ...
    try:
        ...
        status, body = await http(port, "GET", "/state")
        assert set(body) == {"topic", "ts", "speaking"}, "the page's /state is unchanged"
...
async def test_without_other_surfaces_the_relay_says_so():
    ...
    task = asyncio.create_task(serve(open_doc, 47824, log=lambda *_: None))
    await asyncio.sleep(0.2)
    try:
