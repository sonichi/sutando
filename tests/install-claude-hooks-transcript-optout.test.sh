#!/usr/bin/env bash
# SUTANDO_HOOKS_OMIT_TRANSCRIPT_ARCHIVE=1 omits the transcript-archive hook and
...
arm() {
    ...
    # The installer targets the core's own config dir; with no scripts/ in the
    # fixture it falls back to <repo>/workspace/.claude-sutando.
    mkdir -p "$t/workspace/.claude-sutando"
    echo '{}' > "$t/workspace/.claude-sutando/settings.json"
    if [ -n "$optout" ]; then
    ...
    python3 - "$t/workspace/.claude-sutando/settings.json" <<'PY'
import json, sys
d = json.load(open(sys.argv[1]))
c = [h.get("command", "") for g in d.get("hooks", {}).get("PreCompact", [])
     for h in g.get("hooks", [])]
print(sum("logs/conversations/" in x for x in c), len(c))
PY
    rm -rf "$t"
}
