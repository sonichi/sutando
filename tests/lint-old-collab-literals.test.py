...
now = mod.counts(repo)
check("four spellings counted (room_doc_client keeps counting), the two document lookalikes are not",
      now.get("a.py") == 4, repr(now))
...
check("--update exits 0 and reports the totals", rc == 0 and "3 files, 7 occurrences" in out, out)
written = json.loads((repo / "scripts" / "old-collab-literals.baseline.json").read_text())
check("baseline holds file -> count", written == {"a.py": 4, "docs/b.md": 1, "env.sh": 2}, repr(written))
rc, out = run(mod, repo)
check("unchanged tree is ok", rc == 0 and out.startswith("old-collab-literals: ok (7 occurrences in 3 files)"), out)
...
check("the grown file is named with baseline -> now", "  a.py: 4 -> 5" in out, out)
...
subprocess.run(["git", "-C", str(repo), "add", "-A", "--", "a.py", "new.py"], check=True)
rc, out = run(mod, repo)
check("shrinking is ok and says how much", rc == 0 and "3 fewer than the baseline" in out, out)
