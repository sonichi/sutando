...
class PoolLeadPinTests(unittest.TestCase):
    ...
    def test_pinned_room_routes_to_its_worker(self):
        # core-a would win the least-loaded tie (lexicographic); the pin
        # must send the room's task to core-b instead.
        self.lead.pin_room("chan-P", "core-b")
        self._task("task-p1.txt", channel="chan-P")
        self.assertEqual(self.lead.sweep(), [("task-p1.txt", "core-b")])

    def test_dead_pinned_worker_loans_task_but_keeps_pin(self):
        self.lead.pin_room("chan-P", "core-b")
        self.alive["core-b"] = False
        self._task("task-p2.txt", channel="chan-P")
        self.assertEqual(self.lead.sweep(), [("task-p2.txt", "core-a")])
        row = self._disk_table()["chan-P"]
        self.assertEqual(row["instance"], "core-b",
    ...
    def test_recovered_pinned_worker_gets_its_room_back(self):
        ...
        self.lead.sweep()
        self.alive["core-b"] = True
        self._task("task-p4.txt", channel="chan-P")
        picks = dict(self.lead.sweep())
        self.assertEqual(picks["task-p4.txt"], "core-b")
    ...
    def test_wedge_release_spares_pinned_row(self):
        ...
        self.assertEqual(self._disk_table()["chan-P"]["instance"], "core-b",
    ...
    def test_wedge_release_still_drops_unpinned_row(self):
        ...
        self.lead._save_affinity(
            {"chan-A": {"instance": "core-b", "ts": self.lead.now()}})
        stuck = self.tasks / "task-p7.assigned-core-b.txt"
    ...
    def test_unpin_keeps_binding_but_restores_auto_rehoming(self):
        ...
        self.lead.sweep()
        self.assertEqual(self._disk_table()["chan-P"]["instance"], "core-a",
    ...
    def test_sweep_save_merges_around_a_concurrent_pin(self):
        ...
        def racing_load():
            ...
            if calls["n"] == 2:  # the merge's locked re-read
                real_save({"chan-Q": {"instance": "core-b", "ts": 9.0,
                                      "pinned": True}})
            return real_load()
...
class PoolBindCliTests(unittest.TestCase):
    ...
    def test_pin_list_unpin_round_trip(self):
        self.assertEqual(self.cli.main(["pin", "chan-X", "core-2"],
                                       workspace=self.ws), 0)
        table = json.loads(
            (self.ws / "state" / "pool" / "affinity.json").read_text())
        self.assertEqual(table["chan-X"]["instance"], "core-2")
