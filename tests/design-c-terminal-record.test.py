...
with tempfile.TemporaryDirectory() as td:
    ...
    tmp = b2.root / "tmp" / f"{k2}{SEP}{TERMINAL_TAG}{SEP}{time.time_ns()}.json"
    ...
    # ── legacy filename-format archive entry counts as terminal ────────
    b4 = fresh(td, "legacy")
    b4.publish("item-4", b"p")
    tok4 = b4.claim("item-4", "w0")
    k4 = _safe_key("item-4")
    (b4.root / "archive" / f"{k4}{SEP}w0{SEP}1{SEP}2{SEP}3{SEP}9").write_text("")
    rep4 = b4.recover()
    check(k4 in rep4.retired,
          "legacy archive entry reads as terminal (claim retired, no redrive)")
    check(b4.terminal_record("item-4") is None,
          "legacy entry yields no receipt (terminal-without-receipt)")
