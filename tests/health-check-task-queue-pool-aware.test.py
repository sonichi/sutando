...
class PoolHeldQueue(unittest.TestCase):
    ...
    def test_a_working_pool_is_not_reported_stuck(self):
        ...
        for i, n in enumerate(["a.assigned-worker-1", "b.claimed-worker-2",
                               "c.assigned-worker-3", "d.claimed-worker-4"]):
    ...
    def test_a_pool_hold_that_never_moves_is_not_ok(self):
        ...
        self._task("task-z.claimed-worker-9.txt", age_s=6 * 3600)
        r = self.hc.check_task_queue()
        self.assertEqual(r["status"], "warn", r["detail"])
        self.assertIn("claimed:worker-9", r["detail"])
    ...
    def test_assigned_is_gated_the_same_as_claimed(self):
        ...
        self._task("task-y.assigned-worker-8.txt", age_s=6 * 3600)
        r = self.hc.check_task_queue()
        self.assertEqual(r["status"], "warn", r["detail"])
        self.assertIn("assigned:worker-8", r["detail"])
    ...
    def test_only_the_stale_holds_are_named(self):
        ...
        self._task("task-fresh.claimed-worker-1.txt", age_s=60)
        self._task("task-stale.claimed-worker-9.txt", age_s=6 * 3600)
        r = self.hc.check_task_queue()
        self.assertEqual(r["status"], "warn", r["detail"])
        self.assertIn("1 of 2", r["detail"])
        self.assertIn("worker-9", r["detail"])
    ...
    def test_a_hold_that_vanishes_mid_sweep_is_skipped_not_fatal(self):
        ...
        self._task("task-gone.claimed-worker-9.txt", age_s=6 * 3600)
        self._task("task-stays.claimed-worker-8.txt", age_s=6 * 3600)
        real_stat = Path.stat
        target = self.ws / "tasks" / "task-gone.claimed-worker-9.txt"
        ...
        with unittest.mock.patch.object(Path, "stat", flaky):
            r = self.hc.check_task_queue()
        self.assertEqual(r["status"], "warn", r["detail"])
        self.assertIn("worker-8", r["detail"])
        self.assertNotIn("worker-9", r["detail"])
    ...
    def test_unassigned_backlog_warns_even_beside_pool_work(self):
        ...
        for i in range(4):
            self._task(f"task-plain{i}.txt", age_s=600 + i)
        self._task("task-held.claimed-worker-4.txt", age_s=900)
        r = self.hc.check_task_queue()
        self.assertEqual(r["status"], "warn", r["detail"])
    ...
    def test_the_detail_names_who_holds_what(self):
        self._task("task-x.claimed-worker-2.txt", age_s=600)
        self._task("task-y.assigned-worker-3.txt", age_s=600)
        d = self.hc.check_task_queue()["detail"]
        self.assertIn("claimed:worker-2", d)
        self.assertIn("assigned:worker-3", d)
...
class PoolHeldStuckVanishing(unittest.TestCase):
    ...
    def test_only_files_past_the_age_are_stuck(self):
        old = self._aged("task-old.claimed-worker-1.txt", 600)
        fresh = self._aged("task-new.claimed-worker-1.txt", 5)
    ...
    def test_a_vanished_file_is_skipped_and_the_scan_continues(self):
        gone = self.ws / "tasks" / "task-gone.claimed-worker-1.txt"
        old = self._aged("task-old.claimed-worker-1.txt", 600)
