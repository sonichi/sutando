...
# The safety of committing before the pull rests on _enforce_carrier_set_pre
# (which itself runs generate_exclude) running BEFORE the add: reordering
# would let an out-of-carrier OR newly-denied-but-tracked path ride into the
# vault with every behavioural check above still green. Pin the order.
...
[ -n "$ex_line" ] && [ -n "$add_line" ] && [ "$ex_line" -lt "$add_line" ] \
  && ok "_enforce_carrier_set_pre runs BEFORE git add inside _commit_local_pre_pull (order pinned)" \
  || bad "_enforce_carrier_set_pre must precede git add in _commit_local_pre_pull (ex=$ex_line add=$add_line)"
