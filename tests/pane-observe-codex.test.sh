...
open; rc=$(wait_ --model gpt-5.6-luna --effort xhigh); [ "$rc" = 0 ] && [ "$(cat "$T/out")" = ACCEPTED ] && [ "$(keys)" = "2 4" ] && ok "4 wait: row digit for the exact id (2), then the effort digit (Extra high = 4) -> ACCEPTED" || fail "4" "rc=$rc keys='$(keys)' $(cat "$T/out" "$T/err")"
open; rc=$(wait_ --model gpt-5.5); [ "$rc" = 0 ] && [ "$(keys)" = "4 Enter" ] && ok "5 no --effort: Enter confirms the reasoning picker's default" || fail "5" "rc=$rc keys='$(keys)'"
