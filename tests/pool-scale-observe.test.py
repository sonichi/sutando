...
class ObserveTests(unittest.TestCase):
    def test_counts_pending_and_in_flight_per_follower(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (tasks / "task-ccc.assigned-core-1.txt").write_text("x")
            (tasks / "task-ddd.claimed-core-2.txt").write_text("x")
            (tasks / "task-eee.claimed-core-2.txt").write_text("x")
            (tasks / "notes.md").write_text("x")          # ignored
            (tasks / "task-cron-1.done").write_text("x")  # ignored: no .txt
            pending, in_flight = observe(tasks, ["core-1", "core-2"])
            self.assertEqual(pending, 2)
            self.assertEqual(in_flight, {"core-1": 1, "core-2": 2})

    def test_unknown_follower_suffix_is_not_counted(self):
        with tempfile.TemporaryDirectory() as td:
            tasks = Path(td)
            (tasks / "task-zzz.claimed-core-9.txt").write_text("x")
            pending, in_flight = observe(tasks, ["core-1"])
            self.assertEqual(pending, 0)
            self.assertEqual(in_flight, {"core-1": 0})

    def test_unreadable_tasks_dir_reads_as_idle(self):
        pending, in_flight = observe(Path("/nonexistent-pool-observe"), ["core-1"])
        self.assertEqual(pending, 0)
        self.assertEqual(in_flight, {"core-1": 0})
...
class DecideGuardTests(unittest.TestCase):
    def test_zero_current_n_holds(self):
        self.assertIsNone(decide(
            pending_unassigned=5, in_flight={"core-1": 9}, current_n=0,
