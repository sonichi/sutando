...
test('Gemini 3.8 is the default live model and latest-search adds Web grounding', () => {
	assert.deepEqual(PRESETS['gemini-3.8'], { model: 'gemini-3.8-live', googleSearch: false });
	assert.deepEqual(PRESETS['latest-search'], { model: 'gemini-3.8-live', googleSearch: true });
	assert.notEqual(PRESETS['no-search'].model, PRESETS['latest-search'].model, 'legacy 3.1 remains available explicitly');
...
test('the tool schema accepts latest-search and still refuses an unknown name', () => {
	...
	assert.equal(schema.safeParse({ preset: 'gemini-3.8' }).success, true);
...
test('switching to latest-search overlays only model + googleSearch', () => {
	...
	assert.equal(next.model, 'gemini-3.8-live');
