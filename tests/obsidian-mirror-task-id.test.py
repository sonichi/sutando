...
Its old greedy `^task-(.+)\.txt$` returned `task-x.claimed-worker-2`, so a claimed
...
class TaskIdFromPath(unittest.TestCase):
    ...
    def test_the_regression_a_greedy_pattern_gets_wrong(self):
        got = om._task_id_from_path(Path("task-abc123.claimed-worker-2.txt"))
        self.assertNotEqual(got, "task-abc123.claimed-worker-2")
