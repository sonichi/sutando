...
SH
chmod +x "$T/bin/tmux"
export PATH="$T/bin:$PATH" TMUX_LOG="$T/tmux.log" CLAUDE_CONFIG_DIR="$T/cfg"
...
case "$R" in "claude-fable-5-1[1m] claude-opus-5 20"*" scripts/switch-model.sh") ok "5 switch record carries model, previous, a dated ts, and the writer";; *) fail "5 record" "$R";; esac
...
echo; [ $fails -eq 0 ] && echo "switch-model: all 11 checks pass" || { echo "switch-model: $fails FAILED"; exit 1; }
