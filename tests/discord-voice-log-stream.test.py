...
- appendOperationalLog uses _opLogStream with appendFileSync fallback
...
def main() -> None:
    ...
    expect(
        ...
        ctx=lines[32] if len(lines) > 32 else "",
    )
    ...
    # 7. appendOperationalLog has appendFileSync fallback
    expect(
        "appendFileSync" in fn_body,
        "discord-voice-server.ts: appendOperationalLog must have appendFileSync fallback for stream-open failure",
        ctx=fn_body[:200] if fn_body else "",
    )

    # 8. mkdirSync for DISCORD_VOICE_LOG dir is called at module init (before function def)
    ...
    # 9. stream error handler set to null on error (graceful degradation)
    expect(
    ...
    # 10. existing mkdirSync calls for DATA/RESULTS/TASKS dirs still present
    ...
    if _FAILURES:
        ...
        print("All 12 structural tests passed.")
