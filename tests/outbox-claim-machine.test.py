#!/usr/bin/env python3
"""Hypothesis ClaimMachine: stateful model of the outbox delivery-claim protocol.

Drives the REAL src/outbox.py (never a copy) with two concurrent drainer
threads whose filesystem boundaries are gated, so Hypothesis chooses the
interleaving at every read/decision boundary — the preemption class that
in-process sequential tests cannot reach (#2985). Invariants:

  I1  at most one drainer believes it holds the item's claim;
  I2  if a drainer believes, the claim file exists and names that drainer;
  I3  an ownership-checked release never destroys a successor's claim
      (subsumed by I1+I2 — the destroyed claim surfaces as a believer
      whose file is gone, or as two believers after the next acquire).

The adapter layer below tolerates historical API shapes (the pre-#2975-review
heads lack drainer_id/force on release) so the same machine can be replayed
against defective heads to measure what it catches; the protocol logic
exercised is always the imported module's own.

Requires hypothesis; exits 0 with a SKIP line when it is unavailable so this
never breaks a bare CI env.
"""
...
except ImportError:  # pragma: no cover - experiment can run without CI dep
...
class Gate:
    ...
    at_gate and blocks; the machine clears it, grants one permit, and waits
    for the worker to reach its next observable state. Interleavings are
    then chosen by the rule sequence, not by OS thread timing."""
...
class GatedPath(type(Path())):
    ...
    def _maybe_pause(self):
        g = GatedPath._gate
        if g is not None and GatedPath._root and                 str(self).startswith(GatedPath._root):
            g.pause()
...
if __name__ == "__main__":
    ...
    seed_arg = os.environ.get("CLAIM_MACHINE_SEED")
    kw = {}
    if seed_arg is not None:
        from hypothesis import seed as _seed  # noqa: F401
