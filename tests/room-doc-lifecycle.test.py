...
try:
    ...
    # In CI a missing dep is a wiring fault: exit 0 leaves the job green.
    import os
    if os.environ.get("CI"):
        print(f"room-doc lifecycle: FAIL — dependencies absent in CI: {exc}\n"
              "  install: pip install -r skills/room-doc/requirements.txt")
        sys.exit(1)
    ...
    async def handler(self, ws):
        ...
        async for raw in ws:
            ...
            if data[0] == YMessageType.SYNC:
                ...
                from pycrdt import Awareness as _A, Doc as _D, read_message
                probe = _A(_D())
                probe.apply_awareness_update(read_message(data[1:]), "t")
                seen = self.awareness_by_socket.setdefault(id(ws), set())
                seen.update(probe.states.keys())
...
async def test_an_invalid_kind_fails_before_it_can_mean_markdown():
    """An empty or malformed kind must not fall back to the default document —
    that is the silent wrong-store failure this skill exists to remove."""
    async with Server() as s:
        for bad in ("", "Board", "../foo", "a" * 33):
            try:
                async with open_room_doc(s.url, "!r:x", "tok", kind=bad):
                    raise AssertionError(f"kind {bad!r} must be refused, not defaulted")
            except RoomDocError as e:
                assert "kind" in str(e), e
        assert not s.paths, f"no connection should have been attempted: {s.paths}"


async def test_a_client_never_republishes_another_peers_presence():
    """The ghost: re-sending a peer's awareness makes this socket a holder of
    that peer's id server-side, so the peer keeps showing as present after it
    leaves. One client cannot expose this — it needs a second one to echo."""
    async with Server() as s:
        async with open_room_doc(s.url, "!r:x", "tok") as a:
            await a.set_presence("agent-a")
            await asyncio.sleep(0.2)
            a_id = a._awareness.client_id
            async with open_room_doc(s.url, "!r:x", "tok") as b:
                await b.set_presence("agent-b")
                b_id = b._awareness.client_id
                # hand A's state to B exactly as the server would
                update = a._awareness.encode_awareness_update([a_id])
                b._awareness.apply_awareness_update(update, "remote")
                await asyncio.sleep(0.4)
                sent_by_b = set()
                for sock, ids in s.awareness_by_socket.items():
                    if b_id in ids:
                        sent_by_b |= ids
                assert a_id not in sent_by_b, (
                    f"B republished A's id {a_id}; server would treat B as a holder "
                    f"of A and keep A present after A leaves. saw {sent_by_b}")
                assert b_id in sent_by_b, "control: B must still publish its own"
