...
async def _chat_async(activity: bool = False, verbose: bool = False,
    ...
    # One-screen DM: one connection multiplexes subscribe + submit + pushes;
    # stdin lines become tasks, results print inline.
...
async def _chat_tui(reader, writer, _send, level=0, agent_id=None) -> None:  # pragma: no cover — tty-only fixed UI; _chat_line is the tested twin
    ...
    def draw_input():
        # Compose box wraps across rows and grows upward; the transcript
    ...
    def emit(line):
        # Wrap long PLAIN lines to width; colored border/label lines pass
        # through (measuring visible width through ANSI isn't worth it).
        _, cols = dims()
        line = line.replace("\n", " ")
    ...
    def on_enter():
        ...
        if text.startswith("/"):
            ...
            # Symmetric with the agent box below so the sent message reads
            # as ONE unit, not text with a detached "(sent)" label.
    ...
    def on_key():
        ...
        if not data:
            ...
            return
        # Unbracketed paste: detect by burst shape (typed Enter = lone
        # trailing newline; paste = interior newlines) — never submit those.
        import time as _t
        core = data[:-1]
        if b"\r" in core or b"\n" in core:
