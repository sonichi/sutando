...
//   1. "Say X, Y, or Z" / "say X or Y" — the trailing cue I actually write
//      at the end of results. Most reliable.
//   2. "DECISION: X / Y / Z" line — structured marker.
// Returns array of short option tokens, or null if no match.
function parseDecisionOptions(text) {
  if (!text) return null;
  // Pattern 1: "Say X, Y, or Z" — consume a line ending with "." or EOL.
  var sm = text.match(/\bSay\s+([^.\n\r]+?)(?:\s*\.|\s*$)/im);
  if (sm) {
    var list = sm[1].trim().replace(/,?\s+or\s+/i, ', ');
  ...
  // Pattern 2: DECISION: line — strip parentheticals + long trailing prose.
  var m = text.match(/DECISION:\s*([^\n\r]+)/i);
  if (m) {
    var opts = m[1].split('/').map(function(p) {
      var s = p.trim().replace(/^\*\*|\*\*$/g, '').trim();
      // Take before em-dash, en-dash, opening paren, or ". "
      var cut = s.split(/\s*[—–(]\s*|\.\s/)[0].trim();
...
document.addEventListener('keydown', function(e) {
  ...
  var input = e.target;
  if (!input.classList || !input.classList.contains('task-action-input')) return;
  if (!input.dataset.taskid) return;
