...
the core's positive `idle` status before touching the interactive input, then
selects the highest-priority pending task from disk (`urgent`, `normal`, `low`,
FIFO within a tier). This keeps a busy core from losing an injected prompt and
prevents scheduled low-priority work from blocking later owner messages.
