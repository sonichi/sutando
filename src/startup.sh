  ...
  if ! "$_vsc_py" -c "import detect_secrets" >/dev/null 2>&1; then
    ...
    case "$_vsc_py" in
      */engine/runtime/python/*)
        # Twin of secret_scanner.install_hint: a pip install into the desktop's
        # bundled python is erased by the next engine update (seen twice on one
        # host, 2026-09-16/17); the app build vendors it (fetch-scanner-deps.sh).
