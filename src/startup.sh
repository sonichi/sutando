...
# Usage: bash src/startup.sh [--headless]

set -e

# --headless (or SUTANDO_HEADLESS=1) suppresses the optional macOS menu-bar app
# for deployments with no GUI. Default stays ON so an OSS install is unchanged.
for _arg in "$@"; do
  case "$_arg" in
    --headless) SUTANDO_HEADLESS=1 ;;
  esac
done
export SUTANDO_HEADLESS="${SUTANDO_HEADLESS:-0}"
...
# 5b. Optional macOS menu-bar app (default ON; opt out with --headless or
# SUTANDO_HEADLESS=1 for service/container deployments that have no GUI).
if [ "${SUTANDO_HEADLESS:-0}" = "1" ]; then
  echo "  → headless: skipping menu-bar app build/launch"
else
