...
case "$__holders" in
  ...
  *)
    while IFS=' ' read -r __hpid __hrole; do
      ...
      # Re-prove the pid is STILL this inbox's watcher immediately before each
      # signal: a dead holder's pid can be recycled onto an unrelated process.
      __still_holder() {
        "$SUTANDO_PY_BIN" "$__REPO_ROOT/src/watcher_identity.py" inbox-holders \
          --inbox "$TASKS_DIR_ABS" --exclude "$$" 2>/dev/null | grep -q "^$1 "
      }
      if __still_holder "$__hpid"; then kill -TERM "$__hpid" 2>/dev/null || true; fi
      for _ in $(seq 1 100); do __holder_live "$__hpid" || break; sleep 0.1; done
      if __holder_live "$__hpid" && __still_holder "$__hpid"; then
        kill -KILL "$__hpid" 2>/dev/null || true
        for _ in $(seq 1 30); do __holder_live "$__hpid" || break; sleep 0.1; done
      fi
      if __holder_live "$__hpid" && __still_holder "$__hpid"; then
        echo "watch-tasks-stream: pid $__hpid did not exit; not starting" >&2
        exit 3
      fi
...
unset -f __holder_live __still_holder 2>/dev/null || true
