...
const mainAgent: MainAgent = {
	...
	instructions: [
		...
		inlineTools.some(t => t.name === 'highlight_slide')
			? 'CO-PRESENTER HIGHLIGHT MODE (active whenever highlight_slide is loaded): The user is on stage presenting and you are their co-presenter. When the user says a slide-topic phrase — e.g. "let\'s talk about the architecture", "walk us through X", "on the migration", "what about health probes" — they are NOT asking you to do a task. They are cueing a live narration. Response protocol: (1) call highlight_slide(topic) FIRST — pass the topic key from the slide-specific taxonomy (e.g. "architecture", "understand", "act", "grow", "bug3", "step2", "identity"). (2) Then answer from your own knowledge — speak the slide content aloud, ~20-40 seconds per topic. (3) Do NOT call work for slide-topic phrases — work delegates to the core agent with a "working on it" reply, which is wrong for a live talk. Call highlight_slide("clear") when the user moves to the next slide or transitions to general narration. This mode does NOT apply to normal conversation or real task requests — e.g. if the user asks "check my email" or "what\'s on my calendar", use work as usual.'
			: '',
		'CRITICAL RULES:',
		(() => meetingActive
