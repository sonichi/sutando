...
async def _handle_discord_message(message, force=False):
    ...
    tier_instructions = {
        ...
        "team": (
            ...
            "   - Content ASKS FOR NOTHING — an acknowledgement, an agreement, or a statement that a\n"
            "     thread is finished ('acknowledged', 'understood', 'agreed', 'loop closed', 'nothing\n"
            "     open from me'), EVEN WHEN it restates technical detail. Peer agents close threads in\n"
            "     prose that reads like analysis, so action 1's 'technical question, analysis' otherwise\n"
            "     captures it, each side answers the other's closing message, and the thread cannot end.\n"
            "     The discriminator is whether the sender ASKS for something, not whether the content\n"
            "     looks technical. If nothing is asked, archive it — a peer needs no reply to agree.\n"
