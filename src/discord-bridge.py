...
async def _handle_discord_message(message, force=False):
    ...
    tier_instructions = {
        ...
        "team": (
            ...
            f"   - Stage 1: codex exec --sandbox read-only -o /tmp/sutando-codex-staging-{{id}}.txt -- {quoted_task}\n"
            f"   - Stage 2: if codex exits 0 AND /tmp/sutando-codex-staging-{{id}}.txt is non-empty: mv /tmp/sutando-codex-staging-{{id}}.txt {RESULTS_DIR}/task-{{id}}.txt (atomic single move; bridge only ever sees a complete file).\n"
        ...
        "other": (
            ...
            f"  Stage 1: codex exec --sandbox read-only --skip-git-repo-check -C /tmp -o /tmp/sutando-codex-staging-{{id}}.txt -- {quoted_task}\n"
            f"  Stage 2: if codex exits 0 AND /tmp/sutando-codex-staging-{{id}}.txt is non-empty: mv /tmp/sutando-codex-staging-{{id}}.txt {RESULTS_DIR}/task-{{id}}.txt (atomic single move).\n"
