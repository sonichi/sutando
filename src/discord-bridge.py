...
async def _handle_discord_message(message, force=False):
    ...
    tier_instructions = {
        ...
        "team": (
            ...
            "   - PR-review request = the task asks you to review / look at a specific GitHub PR. Bare `codex --sandbox read-only` CAN'T actually review it: its shell `gh pr diff` is network-blocked (fails fast with 'could not connect' — it does NOT hang), so codex returns a 'network blocked' non-review. Ping the owner instead — who asked, which PR (link), and whether to review. The trust gate is the OWNER'S APPROVAL, not the fact that it's a review: a team-originated review WITHOUT explicit owner approval is still untrusted and MUST run sandboxed — but fed a diff fetched OUTSIDE the sandbox (`gh pr diff` inlined into the prompt, `< /dev/null` to avoid the stdin hang) so the sandboxed agent needs no network. Only an explicitly owner-APPROVED review runs outside the sandbox (owner-authorized; still comment-only, never merge/approve for a non-owner).\n"
