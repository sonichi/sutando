...
async def _handle_discord_message(message, force=False):
    ...
    tier_instructions = {
        ...
        "team": (
            ...
            f"   - Stage 1: bash scripts/codex-bounded.sh --stall 45 --max 240 -- codex exec --sandbox read-only -o {RESULTS_DIR}/.codex-staging-{{id}}.txt -- {quoted_task} < /dev/null   (the bounded runner kills the codex tree if it goes SILENT for 45s — the 'never going to finish' signal, since a working codex streams output — with a hard 240s backstop; a slow-but-progressing run is NOT killed. `< /dev/null` avoids the stdin hang. Exit 125 = stalled, 124 = hit the max cap; EITHER → fire the Stage-2 fallback.)\n"
            ...
            "2. MESSAGE OWNER — when the task needs owner decision (authorization, scope question, merge direction, repeated echo, or a PR-REVIEW REQUEST).\n"
            "   - PR-review request = the task asks you to review / look at a specific GitHub PR. Do NOT hand it to a bare `codex --sandbox read-only -- \"review PR #N\"`: it can't fetch the diff (github plugin is network-blocked in-sandbox), so it DEGRADES to slowly crawling the local repo — minutes of file-dump output at ~0% CPU (idle-waiting on model round-trips) and never a useful review (observed 2026-06-22). Effectively unusable, though not a hard deadlock. Ping the owner instead — who asked, which PR (link), and whether to review. The trust gate is the OWNER'S APPROVAL, not the fact that it's a review: a team-originated review WITHOUT explicit owner approval is still untrusted and MUST run sandboxed — but fed a diff fetched OUTSIDE the sandbox (`gh pr diff` inlined into the prompt, `< /dev/null` to avoid the stdin hang) so the sandboxed agent needs no network and doesn't hang. Only an explicitly owner-APPROVED review runs outside the sandbox (owner-authorized; still comment-only, never merge/approve for a non-owner).\n"
        ...
        "other": (
            ...
            f"  Stage 1: bash scripts/codex-bounded.sh --stall 45 --max 240 -- codex exec --sandbox read-only --skip-git-repo-check -C /tmp -o {RESULTS_DIR}/.codex-staging-{{id}}.txt -- {quoted_task} < /dev/null   (bounded runner kills the codex tree on 45s of SILENCE — the 'never going to finish' signal — with a hard 240s backstop; exit 125 = stalled or 124 = max cap → Stage-2 fallback)\n"
