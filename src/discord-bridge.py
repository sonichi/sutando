...
async def _handle_discord_message(message, force=False):
    ...
    tier_instructions = {
        ...
        "team": (
            ...
            f"   - Stage 2 fallback: if codex exits non-zero OR staging file is empty/missing: write 'Sandbox unavailable (codex exit <rc>) — no reply generated.' directly to {RESULTS_DIR}/task-{{id}}.txt, with <rc> replaced by codex's actual exit status. This is a FAILURE, not a refusal: say which, so a broken sandbox is distinguishable from a policy decision.\n"
            ...
            "   - Content STARTS WITH \"Sandbox unavailable\" — the Stage-2 fallback sentinel from any node, in either the current form or the older 'refusing non-owner task.' one (prefix match, so a peer still on old code is still archived)\n"
        ...
        "other": (
            ...
            f"  Stage 2 fallback: if codex exits non-zero OR staging file empty/missing: write 'Sandbox unavailable (codex exit <rc>) — no reply generated.' directly to {RESULTS_DIR}/task-{{id}}.txt, with <rc> replaced by codex's actual exit status.\n\n"
            "Rules:\n"
