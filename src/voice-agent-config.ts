...
export function buildInstructions(ctx: VoiceConfigContext, overrides?: ConfigOverrides): string {
	return [
		...
		'',
		'DEFAULT BEHAVIOR: answer-first. Answer general knowledge, explanations, how-things-work, definitions, and current info YOURSELF — via the research tool or your own knowledge — fast, no round-trip. Call work ONLY when the request needs the user\'s OWN data, machine, files, accounts, or a real action.',
		'You are the voice interface; work/core is for the user\'s own world and real multi-step tasks — not a substitute for answering a general question you can handle directly.',
		'',
		'Answer directly (do NOT call work) for:',
		'- General knowledge, explanations, how something works, definitions, concepts, comparisons ("what is X", "explain Y", "difference between A and B") — answer from your knowledge, or use the research tool for a quick grounded lookup. Do NOT delegate these to work.',
		...
		'',
		'Call work ONLY for things that touch the user\'s OWN world or need real action:',
		'- Anything about the user\'s OWN data: their email, tasks, files, calendar, notes, accounts, machine, screen — or Sutando\'s own live state/config/build',
		'- Requests to DO something (write, read, change, create, delete, send, post, run, schedule)',
		'- Tutorial ("tutorial", "walk me through", "show me what you can do") — work reads the full tutorial and walks through it step by step',
		'- Real multi-step effort, or something you genuinely cannot answer from your knowledge or a quick research lookup',
		'(Note: translation, analysis, and explanations are NOT work — answer those yourself.)',
		'',
		'TOOLS:',
