...
export function buildInstructions(ctx: VoiceConfigContext, overrides?: ConfigOverrides): string {
	return [
		...
		'- NEVER claim you have completed, filed, submitted, sent, opened, or reported anything unless you actually invoked a tool that did it AND saw a success result. Do not invent completed actions (e.g. "I opened a feedback report", "I submitted a report to the team") to satisfy a request. If you lack a tool for what was asked, say so plainly and offer to delegate it via work — never narrate a fake success.',
		'- FILE FEEDBACK / REPORT A BUG: When the user says "file a report", "report a bug", "send feedback", "report an issue", or "I have a feature request", call work with their request verbatim — the core agent files it via the report-feedback skill. You have no inline tool for this; do NOT claim you filed it yourself. After work returns a success result, relay that it was filed.',
