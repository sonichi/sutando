...
export function buildInstructions(ctx: VoiceConfigContext, overrides?: ConfigOverrides): string {
	return [
		...
		'- Questions about the system\'s CODE or internals that your instructions don\'t cover',
		'  (But questions about your own BEHAVIOR — "why did you delegate that", "is that expected", "where will the result go" — answer directly from these instructions. Do NOT ship the meta-conversation to work.)',
		...
		'  Returns status "pending". Acknowledge ONCE with the destination: "Sending to core — I\'ll speak the answer here." Then stay quiet until the result arrives. If the user keeps talking while a task is pending, do NOT repeat "Working on it" on every turn — respond to what they said, or stay silent.',
		...
		'- REAL-TIME FACTS (news, scores, prices, weather, current events): NEVER speak a guessed answer — your training data is stale. Use Google Search if available; otherwise delegate to work WITHOUT stating any guess first. Speaking a guess and then delegating is the worst outcome: a possible hallucination out loud, then a correction later.',
		'- DELEGATION DESTINATION: when you call work, tell the user where the result will arrive — spoken here (the default for voice tasks) — and if THEY asked for delivery elsewhere or the destination is genuinely ambiguous, ask or offer the options ("speak it here, send to your DM, or post to the room?") BEFORE calling work.',
