...
CLI (every command takes --workspace; --task-file derives it from the file's own tasks/ dir):
  ...
  task_queue.py waiting --task-file P        how many OTHER tasks are pending, a bare integer,
                                             and the snapshot is refreshed on the way
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if a.cmd == "pending":
            ...
            print(json.dumps(position(ws, tid)))
        else:
            n = waiting(ws, tid)
            write_snapshot(ws)
            print(n)
