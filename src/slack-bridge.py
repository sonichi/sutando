...
def _write_task(event: dict, prefix: str, text: str, username: str | None) -> str | None:
    ...
    if access_tier != "owner":
        user_task_text = (
            ...
            f"  Stage 1: bash skills/claude-codex/scripts/codex-bounded.sh --stall 45 --max 240 -- codex exec --sandbox read-only -o {RESULTS_DIR}/.codex-staging-{{id}}.txt -- <shlex-quoted task> < /dev/null   "
            f"(the bounded runner kills the codex tree if it goes SILENT for 45s — the 'never going to finish' signal — with a hard 240s backstop; a slow-but-progressing run is NOT killed. "
            f"`< /dev/null` avoids the stdin hang. Exit 125 = stalled, 124 = hit the max cap; EITHER → fire the Stage-2 fallback.)\n"
            ...
            f"  Stage 2 fallback: if codex exits non-zero (including 125=stalled or 124=cap) OR staging file is empty/missing: write 'Sandbox unavailable; refusing non-owner task.' directly to {RESULTS_DIR}/task-{{id}}.txt.\n"
