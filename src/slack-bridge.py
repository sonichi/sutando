...
def _write_task(event: dict, prefix: str, text: str, username: str | None) -> str | None:
    ...
    if access_tier != "owner":
        user_task_text = (
            ...
            f"You MUST delegate to a sandboxed agent "
            f"(`codex exec --sandbox read-only --skip-git-repo-check -- <prompt> < /dev/null` "
            f"— the `< /dev/null` is required or codex waits on stdin and can hang; and assert "
            f"its OUTPUT is non-empty, since it exits 0 on refusal too) "
